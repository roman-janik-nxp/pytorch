...
def _get_edge_to_qspec(
    model: torch.fx.GraphModule,
) -> tuple[dict[EdgeOrNode, QuantizationSpecBase], dict[EdgeOrNode, QuantizationSpecBase]]:
    """Get maps from EdgeOrNode to quantization spec based on annotations on the nodes for inputs and outputs."""
    edge_to_qspec_inputs: dict[EdgeOrNode, QuantizationSpecBase] = {}
    edge_to_qspec_outputs: dict[EdgeOrNode, QuantizationSpecBase] = {}
    for n in model.graph.nodes:
        if hasattr(n, "meta") and "quantization_annotation" in n.meta:
            ...
            for input_to_n, qspec in qa.input_qspec_map.items():
                ...
                edge_to_qspec_inputs[input_edge] = qspec
            for n_to_output, qspec in qa.output_qspec_map.items():
                output_edge = (n, n_to_output)
                edge_to_qspec_outputs[output_edge] = qspec
    return edge_to_qspec_inputs, edge_to_qspec_outputs
...
def _get_edge_to_group_id(
    edge_to_qspec_inputs: dict[EdgeOrNode, QuantizationSpecBase],
    edge_to_qspec_outputs: dict[EdgeOrNode, QuantizationSpecBase]
) -> dict[EdgeOrNode, int]:
    """Map from edge to the group ID, generated from quantization annotations,
    edge with the same group ID should use the same observer/fake_quant instance

    This is applying SharedQuantizationSpec configuration and map each edge to a group
    ...
    Args:
        edge_to_qspec_inputs: Dictionary from edge to the qspec, derived from annotations for inputs
        edge_to_qspec_outputs: Dictionary from edge to the qspec, derived from annotations for outputs
    Returns:
        edge_to_group_id: Dictionary from edge to group_id (int), all edge or node that
    ...
    Example:
        ...
        edge_to_group_id = _get_edge_to_group_id(edge_or_node_to_qspec)
        edge_to_group_id: {
            op1: 1,
            op2: 1,
            (op1, cat1): 1,
    ...
    shared_with_map: dict[EdgeOrNode, EdgeOrNode] = {
        k: k for k in edge_to_qspec_inputs.keys()
    }
    shared_with_map.update({k: k for k in edge_to_qspec_outputs.keys()})
    for input_edge, qspec in edge_to_qspec_inputs.items():
        input_edge_root_qspec = _unwrap_shared_qspec(
            qspec, edge_to_qspec_inputs, shared_with_map
        )
    ...
    for output_edge, qspec in edge_to_qspec_outputs.items():
        assert isinstance(output_edge, tuple)
        _update_shared_with(output_edge, qspec, shared_with_map)

    # now that we get the sharing relations between all edges and nodes, we can assign group ids
    cur_group_id = 0
    edge_to_group_id: dict[EdgeOrNode, int] = {}
    for edge in shared_with_map.keys():
        root = _find_root_edge_or_node(edge, shared_with_map)
        if root not in edge_to_group_id:
            edge_to_group_id[root] = cur_group_id
            cur_group_id += 1
        edge_to_group_id[edge] = edge_to_group_id[root]

    return edge_to_group_id


def _get_obs_or_fq_map(
    edge_to_group_id: dict[EdgeOrNode, int],
    edge_to_qspec: dict[EdgeOrNode, QuantizationSpecBase],
    is_qat: bool,
    ...
    for edge_or_node, qspec in edge_to_qspec.items():
        group_id = edge_to_group_id[edge_or_node]
...
def _maybe_insert_input_observers_for_node(
    ...
    # This is just a workaround for these.
    assert (
...
def _maybe_insert_output_observers_for_node(
    node: Node,
    model: torch.nn.Module,
    named_modules: dict[str, torch.nn.Module],
    graph: Graph,
    ...
    for user_node in orig_users:
        # Insert Observer or FakeQuantize after getitem node, as they are needed to select the correct output
        insert_point_node = user_node if user_node.op == "call_function" and "getitem" in user_node.name else node
        output_edge = (node, user_node)
        if output_edge not in obs_or_fq_map:
            continue
        output_act_obs_or_fq = obs_or_fq_map[output_edge]
        new_output = _insert_obs_or_fq(
            insert_point_node, output_act_obs_or_fq, model, named_modules, graph
        )
        # propagate numeric debug handle from original node to observer/fake_quant node
        if (
            isinstance(insert_point_node, Node)
            and isinstance(new_output, Node)
            and CUSTOM_KEY in insert_point_node.meta
            and NUMERIC_DEBUG_HANDLE_KEY in insert_point_node.meta[CUSTOM_KEY]
        ):
            if CUSTOM_KEY not in new_output.meta:
                new_output.meta[CUSTOM_KEY] = {}
            new_output.meta[CUSTOM_KEY][NUMERIC_DEBUG_HANDLE_KEY] = insert_point_node.meta[
                CUSTOM_KEY
            ][NUMERIC_DEBUG_HANDLE_KEY]

        # Update users of original node to use the output observer
        # instead. For example, change
        #           Node with 1 output              Node with multiple outputs
        #
        #           next_node                                               next_next_node1
        #          /                                                        /
        #   cur_node -> obs                 cur_node -> next_node1 (getitem) -> obs
        #                                          \
        #                                          next_node2 (getitem) -> next_next_node2
        #
        # to
        #
        #                 next_node                                               next_next_node1
        ...
        if user_node.op == "call_function" and "getitem" in user_node.name:
            getitem_user_node = list(user_node.users.keys())[0]
            getitem_user_node.replace_input_with(insert_point_node, new_output)
        else:
            user_node.replace_input_with(insert_point_node, new_output)
...
def _maybe_insert_input_and_output_observers_for_node(
    ...
    output_is_correct_type = (
            "val" in node.meta
            and (isinstance(node.meta["val"], FakeTensor)
                 or (isinstance(node.meta["val"], tuple)
                     and node.meta["val"]
                     and all(isinstance(x, FakeTensor) for x in node.meta["val"])
                     ))
    )
    if not output_is_correct_type:
        return

    # insert new output observers
    _maybe_insert_output_observers_for_node(
...
def prepare(
    ...
    # At the high level we construct a map from EdgeOrNode to an observer_or_fake_quant instance
    # all edges that belong to the same group will use the same instance
    # and when we insert observers we'll just query this map to get the correct observer_or_fake_quant
    # instance
    edge_to_qspec_inputs, edge_to_qspec_outputs = _get_edge_to_qspec(model)
    edge_to_group_id = _get_edge_to_group_id(edge_to_qspec_inputs, edge_to_qspec_outputs)
    edge_to_qspec_inputs.update(edge_to_qspec_outputs)
    obs_or_fq_map = _get_obs_or_fq_map(
        edge_to_group_id, edge_to_qspec_inputs, is_qat
    )
