...
def _unused_constant(node: torch.fx.Node) -> Optional[list[torch.fx.Node]]:
    ...
    looks like:
        ...
        %detach_ : [num_users=?] = call_function[target=torch.ops.aten.detach_.default](args = (%lift_fresh_copy,), kwargs = {})
    ...
    lift_fresh_node = list(node.users.keys())[0]
    if not (
    ...
    detach_node = list(lift_fresh_node.users.keys())[0]
    if not (
